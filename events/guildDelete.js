...
client.on("guildDelete", (guild, message) => {
    client.channels.cache.get(guilds).send(
      `
  I just left a guild with name: **${guild.name}**. A little information about it:
  ...
  **Now we are on ${client.guilds.cache.size} guilds**`,
      client
    );
    setTimeout(() => {
      console.log(`I just left a guild: ${guild.name} | ${guild.id}`);
    }, 1000);
});